...
# Next it will call install.sh script which will install CUDA
# cudnn linux tar file should also be in the same folder as the shell scripts, cudnn_install will unzip and install cudnn
# After this is done, it will create a python virtual env
...
then
  ...
  sudo mv src/${GIT_DIR}/{.[!.],}* src/
  # rmdir $GIT_DIR
  rm -r $GIT_DIR

  echo "Starting install of CUDA"
  source cuda_install.sh

  echo "Starting cudnn install"
  source cudnn_install.sh
else
  echo "Directory exists"
fi
